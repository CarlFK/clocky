...
_targets = [HOUR, MINUTE, SECOND, SWEEP]
...
class Face(ElementWithDyer):
    ...
    def Draw(self, dc):
        ...
        # print (self.pos.x, self.pos.y, self.pos.x, self.pos.y + self.radius)
        
        width = self.radius/20
        dc.SetPen(wx.Pen("blue", width, wx.SOLID))
        # dc.DrawLine(self.pos.x-width/2, self.pos.y, self.pos.x-width/2, self.pos.y - self.radius)
        dc.DrawLine(self.pos.x, self.pos.y, self.pos.x, self.pos.y - self.radius)
...
class SweepHand(Hand):

    def __init__(self, sweep_frequency=10, **kwargs):
        Hand.__init__(self, **kwargs)
        self.sweep_frequency = sweep_frequency
        self.SetPolygon([[-1, -1], [1, -1], [1, 10], [-1, 10]])
...
class HandSet:
    ...
    def __init__(self, parent, h, m, s, w):
        self.parent = parent

        self.hands = [h, m, s, w]
        self.radius = 1
        self.centre = wx.Point(1, 1)
        self.sweep_frequency = w.sweep_frequency
        self.last_second=0
        self.flash_state = False

    def _draw(self, dc, shadow=False):
        ends = [int(x) for x in strftime("%I %M %S", localtime()).split()]
        ends.append(datetime.now().microsecond)

        second = datetime.now().microsecond 
        if second < self.last_second:
            ...
            if self.flash_state:
                ...
                bg = (0,0,0)
            self.parent.Box.Face.SetFillColour(bg)
            self.parent.DrawBox()
